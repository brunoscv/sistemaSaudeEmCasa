...
class Usuarios_model extends CI_Model{
	...
	public function getSetoresId($usuarioId){
		$query = "SELECT * FROM usuarios_setores WHERE usuarios_id = '".$this->db->escape_str($usuarioId)."'";
		$result = $this->db->query($query);
		
		$listaSetores = array();
		foreach($result->result() as $setor){
			$listaSetores[] = $setor->setores_id;
		}
		return $listaSetores;
	}
}
